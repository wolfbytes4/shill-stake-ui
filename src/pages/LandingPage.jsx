// Landing page
const LandingPage = ({ title }) => {
  // Title
  document.title = title;

  return (
    <main className="landing-page">
      {/* Hero section */}
      <section className="hero-section">
        <div className="container">
          <h1>
            Stake Your NFTs and <br className="br" /> Earn Rewards in <br />
            <span>$Shill</span>
          </h1>

          <img
            src="/images/pages/landing-page/hero-section-img-1.png"
            alt=""
            className="hero-section-img-1"
          />

          <img
            src="/images/pages/landing-page/hero-section-img-2.png"
            alt=""
            className="hero-section-img-2"
          />

          <img
            src="/images/pages/landing-page/hero-section-img-3.png"
            alt=""
            className="hero-section-img-3"
          />
        </div>
      </section>

      {/* Texts section */}
      <section className="texts-section">
        <div className="slides">
          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>

          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>

          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>

          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>

          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>

          <div className="slide">
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
            <p>Shillables NFT Club proudly presents: Shillstake.io!</p>
          </div>
        </div>
      </section>

      {/* NFTs section */}
      <section className="nfts-section">
        <div className="container">
          <h2>Stakeable NFTs</h2>
        </div>

        <div className="slides">
          <div className="slide">
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
          </div>

          <div className="slide">
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
          </div>

          <div className="slide">
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
          </div>

          <div className="slide">
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
            <img src="/images/pages/landing-page/nft-soon-img.png" alt="" />
          </div>
        </div>
      </section>

      {/* Collections section */}
      {/* <section className="collections-section">
        <div className="container">
          <img
            src="/images/pages/landing-page/collections-section-img-1.png"
            alt=""
          />
        </div>
      </section> */}

      {/* Stake section */}
      <section className="stake-section">
        <div className="container">
          <h2>Shillables Token Pool and NFT Pools!</h2>

          <div className="boxes">
            <div className="box">
              <div className="content">
                <h3>Shillables Token Pool</h3>

                <p>
                  As long as you have $SHILL in your wallet, you are eligible to
                  stake it to farm for more $SHILL.
                </p>
              </div>

              <img
                src="/images/pages/landing-page/stake-section-img-1.png"
                alt=""
              />
            </div>

            <div className="box">
              <div className="content">
                <h3>NFT Pools</h3>

                <p>
                  Eligible NFTs have their own $SHILL farming pool. If you don't
                  see your favorite secret NFT listed, please message us in
                  discord or twitter to discuss adding it.
                </p>
              </div>

              <img
                src="/images/pages/landing-page/stake-section-img-2.png"
                alt=""
              />
            </div>
          </div>
        </div>
      </section>
    </main>
  );
};

// Export
export default LandingPage;
